...
class DemClient:
    ...
    def RunClient(self):
        ...
        self._log.info(f"Started gRPC DEM Client on {self.serverHost}:{self.port}")
        ...
        server_identity = comm_test_stub.Identify(vehicle_pb2.ClientHello(
            message="DEM Client",
            client_data="No DTC Recorded"
        ))
        self._log.info(f"Com test response - {MessageToDict(server_identity)}")

        # Dem Service Stub
        self.service_stub = vehicle_pb2_grpc.DemCommunicationStub(self._channel)

    def start_dem_service(self):
        """
        Starts dem service with dem communication rpc
        """
        ...
        def request_generator(version: int):
            request = vehicle_pb2.Request(
                ...
                node="DEM_CLIENT",
                resource_names="DEM",
                resource_locators=["telematics", "tailgate"],
                response_nonce="",
                error_detail=None
            )
            yield request
...
def main():
    ...
    CLIENT_LOG_FILE = ROOT_DIR / "logs/client1.log"
    logging.basicConfig(
    ...
    _log = logging.getLogger("Dem Client")
